...
class instances extends model
{
    ...
    private function generaQuery($p_search_query, $p_class_id, $p_fecha_ini, $p_fecha_fin, $p_search_state, $p_parent_inst_id, &$sql_add, &$from_add, $p_mode, $p_relation_id = '') 
	{
        ...
        if ($p_search_query) {
            ...
            $sql_add.=' and v.text_val LIKE "%'.$p_search_query.'%" and v.inst_id = i.id';
    ...
    function instanceList($param_arr) {
        ...
        // Si es un numeric vol dir que estem buscant per ID. Primer busquem per ID i després per la resta
        if (is_numeric($p_search_query)) {
            $ret_id=$this->search_by_id($p_search_query);
        }
        ...
        $first_row=0;
        if ($p_pagina) $first_row=($p_pagina-1)*ROWS_PER_PAGE;
        ...
        $ret_all=parent::get_data($sql);
        if(!$ret_all) return $ret_id;
        $volta=0;
        foreach ($ret_all as $r) {
            if (isset($ret_id) && $volta==0) {
                $ret=$ret_id;
                $volta=1;
            }
            $ret[]=$r;
