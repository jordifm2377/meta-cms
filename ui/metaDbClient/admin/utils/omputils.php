...
{// CONVERT TO TINY URL
	...
	function get_add_to_twitter($p_inst_id, $p_class_id) {
		...
		$javascript = "<script language=\"javascript\" type=\"text/javascript\">
					...
					function do_twit()
					{
						...
						{
							$.get ('".URL_APLI."/admin/utils/twit.php', {message : singleValues}, function(data){
	...
	function getClassName($p_id) {
		if (isset($_SESSION['classes_cache'][$p_id])) return $_SESSION['classes_cache'][$p_id];
		return '';
	}
...
//    if ($valor)
  ...
  if ($mandatories)
  {// Tenim algun mandatory, continuem mirant
  
  //print_r($mandatories); die();
    if ($informats)
	{// Tenim algun informat, comprovem si es un subconjunt dels mandatories
      $diff=array_diff($mandatories, $informats);
      return (count($diff)==0);
	}
	else
	{// Tenim mandatories xo no informats, error directe
	  return false;
	}
  }  
  else
  {// No tenim cap mandatory, no te sentit fer el check !
    return true;
  }
 }
 ...
 function html_item_stack_prefix($p_fila_ant, $p_columna_ant, $p_fila, $p_columna, $p_mode, $p_last = 1)
 {
   ...
   if ($p_columna_ant!=-1) 
   {
     if ($p_columna_ant!=$p_columna)
     {// Cambio de columna
       if ($p_fila_ant==$p_fila) 
       {// En la misma fila
         $ret .= "</p></div><div class='columna'><p>";
       }
       elseif ( $p_last == 0)  //$p_fila_ant==$p_fila &&
       {// En la misma fila
         $ret .= "</p></div></div>";
       }

       else
       {// Canvi de columna i de fila
         $ret .= "</p></div></div><div class='fila' rel='". $p_fila."'><div class='columna'><p>";
       }
     }
     else
     {// Estamos en la misma columna
       if ($p_fila_ant!=$p_fila && $p_last != 0) 
       {// Canvi nomes de fila
         $ret .= "</p></div></div><div class='fila' rel='". $p_fila."'><div class='columna'><p>";
       }
     }
