...
class template
{
	...
	function pinta_paginacion($p_pagina, $p_action, $p_params, $p_num_rows, $conta)
	{
		...
		if ($last_row+1 == 0) // Comprovacio del numero de rows per si no n'hi ha cap
			$res.='<h2>'.getMessage('paginacion_registros_del').' 0 '.getMessage('paginacion_al').' '.($last_row+1).' de '.$conta.'</h2>';

		else // Perfecte tenim mes de 0 rows
			$res.='<h2>'.getMessage('paginacion_registros_del').' '.($first_row+1).' '.getMessage('paginacion_al').' '.($last_row+1).' de '.$conta.'</h2>';
                 $res.='<ul>';
